...
>nul findstr /v "$" "%~nx0" && (
echo:
echo 错误：脚本存在LF行结尾问题，或者脚本末尾缺少空行。
echo:
...
echo 项目仅支持 Windows 7/8/8.1/10/11 及其对应的服务器版本。
goto MASend
)

for %%# in (powershell.exe) do @if "%%~$PATH:#"=="" (
%nceline%
echo 系统中没有安装 Powershell。
echo 正在中止...
goto MASend
)
...
echo:             [4] Online KMS  ^|  Windows / Office  ^|   180 天
...
echo:       ______________________________________________________________
echo:
call :_color2 %_White% "          " %_Green% "输入菜单项 [1,2,3,4,5,6,7,8,0] :"
echo:                                           
echo:                         Translation by CMontage
...
echo:             [3] 激活状态 [vbs]
echo:
echo:             [4] 下载官方 Windows / Office
echo:             __________________________________________________      
echo:                                                                     
echo:             [0] 返回主菜单
echo:       ______________________________________________________________
echo:
call :_color2 %_White% "           " %_Green% "输入选择的菜单项 [1,2,3,4,0] :"
choice /C:12340 /N
set _erl=%errorlevel%
...
echo:           ________________________________________________________
echo:  
call :_color2 %_White% "           " %_Green% "输入选择的菜单项:"
...
call :_color %Green% "$OEM$ 文件夹成功在桌面创建。"
)
echo "%_oem%" | find /i "KMS38" 1>nul && (
echo:
echo 使用 KMS38 方式激活服务器 Cor/Acor 版本 ^(非 GUI 版本^),
echo 访问页面 %mas%oem-folder
)
...
if %~z0 GEQ 200000 (
set "_exitmsg=返回"
set "_fixmsg=返回“主菜单”，选择“故障排除”并运行“修复许可”选项。"
) else (
set "_exitmsg=退出"
...
echo 如果您对Powershell做了限制，请撤消这些更改。
echo:
echo 访问帮助页面. %mas%troubleshoot
goto dk_done
)
...
call :dk_color %Red% "启动 [sppsvc] 服务, 重启过程可能需要很长时间..."
echo:
)
...
echo 检查操作系统信息                        [%winos% ^| %fullbuild% ^| %arch%]
...
if !errorlevel!==0 (set _int=1&set ping_f= 但是 Ping 失败)
)
...
if defined updatesblock call :dk_color %Gray% "正在检查 Windows Update 阻止程序        [发现]"

if defined applist echo: %serv_e% | find /i "wuauserv" %nul% && (
call :dk_color %Blue% "Windows Update Service [wuauserv] 未运行。如果您已禁用，请启用它。"
...
call :dk_color %Gray% "检查应用商店应用拦截              [发现]"
)
...
if %errorcode% EQU 0 (
call :dk_refresh
echo 安装通用产品密钥                 [%key%] [成功]
) else (
call :dk_color %Red% "安装通用产品密钥          [%key%] [失败] %errorcode%"
...
call :dk_color2 %Blue% "访问帮助页面" %_Yellow% " %mas%troubleshoot"
goto :dl_final
)
...
call :dk_color %Red% "生成 GenuineTicket.xml           [失败，进程中止]"
echo [%encoded%]
if exist "%tdir%\Genuine*" del /f /q "%tdir%\Genuine*" %nul%
goto :dl_final
) else (
...
set error=1
set rebuildinfo=1
call :dk_color %Red% "检查 ClipSVC tokens.dat             [未发现]"
)

%_xmlexist% (
set error=1
set rebuildinfo=1
call :dk_color %Red% "安装 GenuineTicket.xml            [失败 clipup -v -o]"
)

if exist "%ProgramData%\Microsoft\Windows\ClipSVC\Install\Migration\*.xml" (
set error=1
set rebuildinfo=1
call :dk_color %Red% "检查 Ticket Migration               [失败]"
)
...
call :dk_color2 %Blue% "访问帮助页面" %_Yellow% " %mas%licensing-servers-issue"
)
)
...
call :dk_color %Red% "删除注册表项                     [失败] [!_ident!]"
) || (
echo 删除注册表项                     [成功] [!_ident!]
)
...
set error=1
echo:
call :dk_color %Red% "检查网络问题                [发现] %error_code%"
call :dk_color2 %Blue% "访问帮助页面" %_Yellow% " %mas%licensing-servers-issue"
)
)
...
call :dk_color %Blue% "使用 KMS38 激活 选项。"
) else (
...
if %osSKU%==175 call :dk_color %Red% "%winos% 在非 azure 平台不支持激活。"
...
call :dk_color %Blue% "重新启动系统以修复禁用的服务错误。"
set showfix=1
)
)
...
set error=1
call :dk_color %Red% "正在检查Windows安装状态            [%imagestate%]"
echo "%imagestate%" | find /i "RESEAL" %nul% && (
set showfix=1
call :dk_color %Blue% "系统在审核模式下运行。请以正常模式运行。"
)
)
...
set error=1
set showfix=1
call :dk_color2 %Red% "检查 WinPE                          " %Blue% "[系统在 WinPE 模式下运行。请以正常模式运行。]"
)
...
set error=1
set wpaerror=1
call :dk_color %Red% "检查WPA注册表错误             [%wpainfo%]"
) || (
echo 正在检查WPA注册表计数             [%wpainfo%]
)
...
if %dism_error% NEQ 0 (
call :dk_color %Red% "检查 DISM                           [未响应] %dism_error%"
)
...
set error=1
set showfix=1
call :dk_color %Red% "检查 评估包                  [非评估许可证安装在评估 Windows中]"
...
call :dk_color %Red% "正在检查版本号                   [在注册表中未发现]"
) else (

if not exist "%SystemRoot%\System32\spp\tokens\skus\%osedition%\%osedition%*.xrm-ms" (
set error=1
call :dk_color %Red% "检查许可证文件                  [未发现] [%osedition%]"
)

if not exist "%SystemRoot%\Servicing\Packages\Microsoft-Windows-*-%osedition%-*.mum" (
set error=1
call :dk_color %Red% "检查程序包文件                   [未发现] [%osedition%]"
)
)
)
...
if %error_code% NEQ 0 (
set error=1
call :dk_color %Red% "检查 slmgr /dlv                     [未响应] %error_code%"
)
...
call :dk_color %Red% "检查 SPP 注册表项               [找到不正确的模块 ID]"
call :dk_color %Blue% "Possibly Caused By Gaming Spoofers. 帮助: %mas%troubleshoot"
set error=1
set showfix=1
)
...
set error=1
call :dk_color %Red% "检查 SPP tokens.dat                 [未发现] [%tokenstore%\]"
)
...
set error=1
set showfix=1
call :dk_color %Red% "检查 sppsvc.exe 文件                [未发现]"
)
...
set error=1
set showfix=1
call :dk_color %Red% "检查 SPP 权限                [发现错误]"
call :dk_color %Blue% "%_fixmsg%"
)
...
set showfix=1
call :dk_color %Blue% "%_fixmsg%"
if not defined permerror call :dk_color %Blue% "如果激活仍然失败，则运行修复 WPA 注册表选项。"
)

if not defined showfix if defined wpaerror (
set showfix=1
call :dk_color %Blue% "如果激活失败，则返回主菜单，选择“疑难解答”并运行“修复WPA注册表”选项。"
)
...
%eline%
echo 检测到不支持的操作系统版本 [%winbuild%].
echo Ohook 激活 需要高于 Windows 8 或之后的服务器版本。
goto dk_done
)
...
for /f "tokens=6-7 delims=[]. " %%i in ('ver') do if "%%j"=="" (set fullbuild=%%i) else (set fullbuild=%%i.%%j)
echo 检查系统信息                        [%winos% ^| %fullbuild% ^| %osarch%]
...
echo:
call :dk_color %Red% "检查不支持的Office     [ %o14msi%%o14c2r%%o16uwp%]"
)
...
call :dk_color %Red% "正在检查ClickToRun服务             [未发现, Office 16.0 文件]"
set o16c2r=
set error=1
)
...
call :dk_color %Red% "正在检查ClickToRun服务             [未发现, Office 15.0 文件]"
set o15c2r=
set error=1
)
...
echo:
if not "%o14msi%%o14c2r%%o16uwp%"=="" (
call :dk_color %Red% "检查支持的Office       [未发现]"
) else (
...
echo:
echo 您只安装了Office仪表板应用程序, 需要安装完整的Office版本。
)
echo:
call :dk_color %Blue% "从下面链接下载并安装Office，然后重试。"
echo:
echo %mas%genuine-installation-media.html
goto dk_done
)
...
call :dk_color %Gray% "检查多个Office安装        [找到. 最好只安装一个版本]"
)
...
echo:
echo 激活 Office 15.0 %_oArch% C2R...

if not defined _oIds (
call :dk_color %Red% "检查已安装的产品             [找不到产品ID， 中止激活...]"
set error=1
goto :starto16c2r
)
...
echo:
echo 激活 Office 16.0 %_oArch% C2R...

if not defined _oIds (
call :dk_color %Red% "检查已安装的版本             [找不到产品。正在中止激活...]"
set error=1
goto :startmsi
)
...
if not defined _sid (
call :dk_color %Red% "正在检查用户帐户SID              [未发现]"
)
...
if defined sub_next echo 正在移除 Office vNext 锁定             [成功]
...
if %upk_result%==1 echo 卸载其他/宽限期密钥           [成功]
if %upk_result%==2 call :dk_color %Red% "卸载其他/宽限期密钥           [失败]"
...
echo:
if not defined error (
call :dk_color %Green% "Office已永久激活。"
...
call :dk_color %Blue% "如果Office应用程序正在运行，请关闭它们，然后重试。"
) else (
call :dk_color %Green% "已成功卸载 Ohook 激活。"
)
)
echo __________________________________________________________________________________________
...
if %errorcode% EQU 0 (
call :dk_refresh
echo 安装通用产品密钥          [%_key%] [%_prod%] [%_lic%] [成功]
) else (
call :dk_color %Red% "安装通用产品密钥          [%_key%] [%_prod%] [Failed] %errorcode%"
if not defined error (
call :dk_color %Blue% "%_fixmsg%"
set showfix=1
)
...
call :dk_color %Gray% "安装丢失的许可证文件        [Office %oVer%.0 %_prod%] [成功]"
exit /b
)
...
call :oh_actids
echo "!oapplist!" | find /i "!_actid!" %nul1% && (
call :dk_color %Gray% "正在安装丢失的许可证文件        [Office %oVer%.0 %_prod%] [成功 使用 /ilc 方法]"
) || (
set error=1
call :dk_color %Red% "正在安装丢失的许可证文件        [Office %oVer%.0 %_prod%] [失败]"
)
...
echo 将系统的sppc.dll符号链接到         ["%_hookPath%\sppcs.dll"] [成功]
echo 正在将自定义%_hook%提取到          ["%_hookPath%\sppc.dll"] [成功]
) else (
set error=1
call :dk_color %Red% "符号链接系统 sppc.dll             [失败]"
call :dk_color %Red% "正在提取自定义 %_hook%            [失败]"
echo ["%_hookPath%\sppc.dll"]
echo:
call :dk_color %Blue% "关闭包括Outlook在内的所有Office应用程序，然后重试。"
call :dk_color %Blue% "如果问题仍未解决，请重新启动系统并重试。"
)
...
set error=1
set ierror=1
call :dk_color %Red% "修改自定义哈希值 %_hook%     [失败]"
) else (
echo 修改自定义哈希值 %_hook%     [成功]
)
)
...
set error=1
call :dk_color %Red% "正在检查脚本中的产品              [Office %oVer%.0 %%# 在脚本中未找到]"
call :dk_color %Blue% "请确保您正在使用的是最新MAS脚本。"
)
)
...
echo:
echo 正在激活 Office %1.0 %_oArch% MSI...
...
%eline%
echo 检测到不支持的操作系统版本 [%winbuild%]。
echo KMS38激活支持Windows 10/11/Server 14393及更高版本。
goto dk_done
)
...
echo         ____________________________________________________________
echo: 
call :dk_color2 %_White% "              " %_Green% "输入选择的菜单项 [1,2,0]"
choice /C:120 /N
set _el=!errorlevel!
if !_el!==3  exit /b
...
echo:
echo 错误代码: %errorlevel%
call :dk_color %Red% "无法启动 [sppsvc] 服务，进程的其余部分可能需要很长时间..."
echo:
)
...
echo:
echo 转到附加菜单中的 更改 Windows 版本 选项。
) else (
echo 无法激活评估版本。
echo 下载并安装完整版本的 %winos%
echo:
echo 下载页面,
echo %mas%genuine-installation-media.html
)
...
%eline%
echo clipup.exe 在服务器 Cor/Acor [非 GUI] 版本中不存在。
echo 这是 KMS38 激活所必需的。
...
call :dk_color %Blue% "必须将Windows更新为19044.2788或更高版本，才能使用KMS38激活方式。"
)
)
...
set error=1
set rebuildinfo=1
call :dk_color %Red% "正在检查 ClipSVC tokens.dat             [未发现]"
)
...
set "_title=Online KMS 激活 %masver%"
set _gui=
...
if defined _oldtsk       call :_color2 %_White% "              [4] 安装自动续期      " %_Red% "[旧版 已安装]"
...
call :_color2 %_White% "              [6] 启用调试模式         " %_Red% "[Yes]"
)
if %vNextOverride% EQU 1 (
if %sub_next% EQU 1 (
call :_color2 %_White% "              [7] 覆盖 Office vNext    " %_Red% "[Yes]"
) else (
echo               [7] 覆盖 Office vNext    [Yes]
)
) else (
if %sub_next% EQU 1 (
call :_color2 %_White% "              [7] 覆盖 Office vNext    " %_Yellow% "[No]"
) else (
echo               [7] 覆盖 Office vNext    [No]
)
)
echo.              _______________________________________________       
echo.
...
echo.       ______________________________________________________________
echo.
call :_color2 %_White% "           " %_Green% "输入选择的菜单项 [1,2,3,4,5,6,7,0]"
choice /C:12345670 /N
set _el=%errorlevel%
...
set "_mO21a=检测到 Office 2021 C2R 零售版已激活"
set "_mO19a=检测到 Office 2019 C2R 零售版已激活"
set "_mO16a=检测到 Office 2016 C2R 零售版已激活"
set "_mO15a=检测到 Office 2013 C2R 零售版已激活"
set "_mO21c=检测到 Office 2021 C2R 零售版无法转换为批量版"
set "_mO19c=检测到 Office 2019 C2R 零售版无法转换为批量版"
set "_mO16c=检测到 Office 2016 C2R 零售版无法转换为批量版"
set "_mO15c=检测到 Office 2013 C2R 零售版无法转换为批量版"
set "_mO14c=检测到 Office 2010 C2R 零售版不被支持"
set "_mO14m=检测到 Office 2010 MSI 零售版不被支持"
set "_mO15m=检测到 Office 2013 MSI 零售版不被支持"
set "_mO16m=检测到 Office 2016 MSI 零售版不被支持"
set "_mOuwp=检测到 Office 365/2016 UWP版不被支持"
...
if %_Debug% EQU 0 (
  ...
  echo.
  echo 正在调试模式下运行。。。
  if not defined _args (echo 完成后，窗口将关闭) else (echo 请稍候...)
  echo.
  echo 将调试日志写入:
...
if defined _unattended (
echo.
call :_color %_Red% "互联网未连接, 持续尝试这个过程。"
) else (
%eline%
echo 互联网未连接。
echo:
...
if %ActWindows% EQU 0 (
  ...
  if %SSppHook% EQU 0 (
    ...
    echo.&echo  检查 KMS 激活 ID 失败。 &call :检查WS
    exit /b
    )
  )
)
...
if %winbuild% GEQ 9200 (
  if %OffUWP% EQU 0 (echo.&echo 未检测到已安装 Office 2013-2021...) else (echo.&echo %_mOuwp%)
  exit /b
  )
if %winbuild% LSS 9200 (if %loc_off14% EQU 0 (echo.&echo 未检测到已安装 Office %aword% ...&exit /b))
)
...
set "_key="
set "_qr=%_zz7% %spp% %_zz2% %_zz5%ID='%app%'%_zz6% %_zz3% Name %_zz8%"
if %ESU_ADD% EQU 0 for /f "tokens=2 delims==" %%x in ('%_qr%') do echo 安装 Key: %%x
if %ESU_ADD% EQU 1 for /f "tokens=2 delims==f" %%x in ('%_qr%') do echo 安装 Key: %%x
set ESU_ADD=0
call :keys %app%
...
if %ESU_ADD% EQU 0 for /f "tokens=2 delims==" %%x in ('%_qr%') do echo 激活: %%x
if %ESU_ADD% EQU 1 for /f "tokens=2 delims==f" %%x in ('%_qr%') do echo 激活: %%x
)
...
if "%_ProductIds%"=="" (
if %_Office15% EQU 0 (echo 错误: Office C2R 产品ID 未检测到&goto :%_fC2R%) else (goto :Reg15istry)
)
if not exist "%_LicensesPath%\ProPlus*.xrm-ms" (
if %_Office15% EQU 0 (echo 错误: Office C2R 许可文件未检测到&goto :%_fC2R%) else (goto :Reg15istry)
)
if not exist "%_Integrator%" (
if %_Office15% EQU 0 (echo 错误：Office C2R 许可证集成商未检测到。&goto :%_fC2R%) else (goto :Reg15istry)
)
...
if "%_Product15Ids%"=="" (
if %_Office16% EQU 0 (echo 错误: Office 2013 C2R 产品ID未检测到&goto :%_fC2R%) else (goto :CheckC2R)
)
...
echo 将不存在的IP地址 10.0.0.10 保留为 KMS 服务器。
) else (
...
if not [%Act_OK%]==[1] (
echo.
echo 如有任何问题, 访问 https://mass%-%grave.dev/troubleshoot
)
...
call :_color %Red% "端口 1688 在互联网连接中被阻止。"
echo.
echo 原因：   可能连接到受限制的互联网 [Office/College]，
echo           或者 防火墙正在阻止连接。
echo.
echo 解决方案：使用其他互联网连接或使用离线 KMS
...
echo ## 注意 ##
echo.
echo 为了确保 Office 程序不会显示非正版提示，,
echo 请运行激活选项一次，以后不要卸载。
echo __________________________________________________________________________________________
)
...
echo __________________________________________________________________________________________
echo.
call :_color %Green% "Online KMS 已成功完全卸载。"
echo __________________________________________________________________________________________
)
...
echo 删除 [任务] Online KMS 激活-仅运行一次
...
echo.
echo 说明:
echo 如果找到互联网连接，激活将每周更新一次。
echo 只会续订已安装的 KMS 许可证。不会将任何许可证转换为 KMS。
echo __________________________________________________________________________________________
echo.
if defined ActTask (
...
echo.
call :_color %Gray% "确保从上一级菜单至少运行一次激活。"
echo __________________________________________________________________________________________
)
...
(
echo   此脚本的用途是使用联机 KMS 激活/续期 您的 Windows/Office 许可证。
echo:
echo   如果创建了续期/激活计划任务，则存在以下内容：
echo:
echo   - 计划任务
echo     激活续期    [续期 / 每周]
echo     激活-仅一次   [激活任务 - 激活后自我删除]
...
echo:
echo   Online KMS 激活脚本是 [MAS] 项目的一部分。
echo:   
echo   主页: mass grave[.]dev
echo   邮箱: windowsaddict@protonmail.com
)>"%_dest%\Info.txt"
exit /b
...
<Task version="1.3" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  <RegistrationInfo>
    ...
    <Version>1.0</Version>
    <Description>在线 K-M-S 激活续期-每周任务</Description>
...
set "License=初始宽限期"
if defined _xpr set "ExpireMsg=初始宽限期结束 %_xpr%"
)
if %LicenseStatus% EQU 3 (
set "License=额外宽限期 (KMS许可证过期或硬件超出容差)"
if defined _xpr set "ExpireMsg=额外宽限期结束 %_xpr%"
)
if %LicenseStatus% EQU 4 (
set "License=非真实宽限期。"
if defined _xpr set "ExpireMsg=非真实宽限期结束 %_xpr%"
)
if %LicenseStatus% EQU 6 (
set "License=延长宽限期"
if defined _xpr set "ExpireMsg=延长的宽限期结束 %_xpr%"
)
if %LicenseStatus% EQU 5 (
set "License=通知"
  if "%LicenseReason%"=="C004F200" (set "LicenseMsg=通知原因: 0xC004F200 (非正品)."
  ) else if "%LicenseReason%"=="C004F009" (set "LicenseMsg=通知原因: 0xC004F009 (宽限期已过期)."
...
if defined ProductKeyChannel echo 产品密钥渠道: %ProductKeyChannel%
...
if defined VLActivationTypeEnabled echo 已配置激活类型: %VLActivationType%
echo.
...
echo.    续期间隔: %VLRenewalInterval% 分钟
...
echo:                                                                      
echo:             [2] 修复组件存储 (Dism RestoreHealth)
echo:             [3] 修复系统文件 (SFC Scannow)
echo:                                                                      
echo:             [4] 修复 WMI
echo:             [5] 修复许可
echo:             [6] 修复 WPA 注册表
...
echo %line%
echo:
echo      Dism 使用 Windows Update 提供修复损坏所需的文件。
echo      这需要5-15分钟或更长时间...
echo %line%
echo:
echo      说明:
...
call :_color2 %_White% "     - " %Gray% "确保Windows更新正常工作。"
echo:
echo %line%
echo:
...
echo:
echo      如果SFC无法修复某个问题，请再次运行该命令，看看是否可以在下一次运行。
...
echo:
call :_color %Gray% "CBS 和主提取日志被复制到桌面上的 AT_logs 文件夹。"
goto :at_back
...
echo:
echo       - 此选项将,
echo            - 停用 Windows 和 Office，可能需要重新激活
echo              如果Windows是通过主板/OEM/数字许可证激活的，那么不要担心
echo:
...
echo %line%
echo:
call :_color %Blue% "重建 ClipSVC 许可证"
echo:

if %winbuild% LSS 10240 (
echo ClipSVC 许可证重建仅在 Win 10/11 和服务器等效版本上受支持。
echo 正在跳过...
goto :cleanvnext
)
...
echo:
echo 正在删除易失性受保护的注册表项...
echo [%RegKey%]
reg query "%RegKey%" %nul% && (
call :_color %Red% "[失败]"
echo 重新启动系统，将自动删除此注册表项。
) || (
echo [成功]
)
...
echo %line%
echo:
call :_color %Blue% "清除 Office vNext 许可证"
echo:
...
if not defined regfound echo 未找到 - Office vNext 注册表项
...
call :_color %Red% "WPA 注册表错误: %wpainfo%"
) || (
echo WPA 注册表计数: %wpainfo%
)
...
if defined token (
echo:
call :_color %Red% "删除失败 .dat files."
echo:
)

echo:
echo 重装系统许可证 [slmgr /rilc]...
cscript //nologo %windir%\system32\slmgr.vbs /rilc %nul%
if %errorlevel% NEQ 0 cscript //nologo %windir%\system32\slmgr.vbs /rilc %nul%
if %errorlevel% EQU 0 (
echo [成功]
...
echo:
echo 运行 osppsvc 服务生成 tokens.dat
...
echo %line%
echo:
call :_color %Blue% "修复 Office 许可证"
echo:
...
if /i "%arch%"=="ARM64" (
echo:
echo 发现 ARM64 Windows 。
echo 你需要在 Windows 设置中为 Office 使用修复选项。
echo:
...
echo:
call :_color %_Yellow% "在弹出一个窗口中，选择修复选项..."
call :_color %_Yellow% "按 任意键 继续..."
echo:
pause %nul1%
)
...
if defined uwp16 (
echo:
echo 注意：正在跳过 Office 16.0 UWP 的修复。
echo       您需要在 Windows 设置中使用重置选项。
echo ________________________________________________________________
echo:
start ms-settings:appsfeatures
)
...
if defined c2r14 (
echo:
echo 注意：正在跳过 Office 14.0 C2R 的修复
...
if exist "%SystemRoot%\Servicing\Packages\Microsoft-Windows-Server*Edition~*.mum" (
%eline%
echo 不建议在 Windows Server 上重建 WMI。中止...
goto :at_back
)
...
if defined _corrupt (
%eline%
echo 未安装 Winmgmt 服务。 正在中止...
goto :at_back
)

echo 正在禁用 Winmgmt 服务...
...
call :_color %Red% "[失败]"
echo:
call :_color %Blue% "建议选择[重启]选项，然后再次应用修复 WMI 选项。"
echo %line%
echo:
choice /C:21 /N /M "> [1] 重启  [2] 还原更改 :"
...
echo:
echo 重新注册 .dll 文件，编译 .mof' .mfl 文件。
...
call :_color %Red% "[未响应]"
echo:
echo 运行 [Dism RestoreHealth] 和 [SFC Scannow] 选项，确保没有错误。
) else (
call :_color %Green% "[工作中]"
)
...
%line%
echo:
call :dk_color %Gray% "您可以将当前版本 [%osedition%] 更改为以下版本之一。"
if defined dismnotworking (
call :dk_color %_Yellow% "注意 - DISM.exe 没有响应。"
if /i "%osedition:~0,4%"=="Core" call :dk_color %_Yellow% "     - 一旦更改为Pro，您将看到更多可供选择的版本选项。"
)
%line%
echo:
...
echo 更改当前版本 [%osedition%] %winbuild% 为 [%targetedition%]
echo:
...
echo:
call :dk_color %Gray% "需要重启才能正确更改版本。"
) else (
...
if %_dismapi%==1 (
echo:
echo 应用DISM API方法使用 %_chan% 密钥 %key%. 请稍等...
echo:
%psc% "$f=[io.file]::ReadAllText('!_batp!') -split ':dismapi\:.*';& ([ScriptBlock]::Create($f[1])) %targetedition% %key%;"
timeout /t 3 %nul1%
echo:
call :dk_color2 %Blue% "访问帮助页面" %_Yellow% " %mas%change_edition_issues"
)
%line%
...
echo 将当前版本 [%osedition%] %winbuild% 更改为 [%targetedition%]
echo:
...
call :dk_color %Blue% "在此阶段必须重新启动系统。"
echo 帮助: %mas%troubleshoot
